...
const DocumentViewer: React.FC<DocumentViewerProps> = ({ document, onDocumentUpdate, editMode }) => {
  const { isConnected, account } = useWallet();
  const [isSignModalOpen, setIsSignModalOpen] = useState(false);
  const [activeFieldId, setActiveFieldId] = useState<string | null>(null);
  const [isProcessing, setIsProcessing] = useState(false);
  const [isAddingSignature, setIsAddingSignature] = useState(false);
  const [pendingField, setPendingField] = useState<Partial<SignatureField> | null>(null);
  ...
  const handleSignConfirm = async (transactionId: string) => {
    ...
    try {
      if (pendingField && account) {
        ...
        document.status = document.signatureFields.length >= 2 ? 'completed' : 'signed';
